...
case "$MODE" in
  backend|all)
    ...
    ;;
  controller)
    run_journal_follow_only | grep -Ei --line-buffered "\\[zwave-runtime\\]|\\[zwave-adapter\\]|ZW0103|The controller is not yet ready|Z-Wave driver is already running|Cannot read properties of undefined \\(reading 'print'\\)"
    ;;
  zwave)
...
Examples:
  ...
  bash deploy/scripts/tail-logs.sh controller   # only new controller logs, no history
